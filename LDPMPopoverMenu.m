...
- (instancetype) initWithTitle:(NSString *) title
                ...
                attachmentView:(UIView *)attachmentView
                        target:(id)target
                        action:(SEL)action {
    NSParameterAssert(title.length || image || attachmentView);
...
- (UIView *) mkContentView
{
    ...
    const CGFloat kMarginTextToAttachView = _menu.marginTextToAttachmentView;
    

    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    CGFloat maxAttachmentWidth = 0;
    ...
    // 计算 attachmentView 的 size
    for (LDPMPopoverMenuItem *menuItem in _menuItems) {
        const CGSize attachmentViewSize = menuItem.attachmentView.size;
        if (attachmentViewSize.width > maxAttachmentWidth) {
            maxAttachmentWidth = attachmentViewSize.width;
        }
    }
    
    for (LDPMPopoverMenuItem *menuItem in _menuItems) {
        
        const CGSize titleSize = [menuItem.title sizeWithAttributes:@{NSFontAttributeName: menuItem.titleFont}];
        const CGSize imageSize = menuItem.image.size;
        const CGSize attachmentViewSize = menuItem.attachmentView.size;
//        const CGFloat itemHeight = MAX(titleSize.height, imageSize.height) + kMarginY * 2;
//        const CGFloat itemWidth = (menuItem.image ? maxImageWidth + kMarginX : 0) + titleSize.width + kMarginX * 2;
        const CGFloat itemHeight = MAX(MAX(titleSize.height, imageSize.height), attachmentViewSize.height) + kMarginY * 2;
        const CGFloat itemWidth = (menuItem.image ? maxImageWidth + kMarginX : 0) + titleSize.width + (menuItem.attachmentView ? maxAttachmentWidth + kMarginTextToAttachView : 0) + kMarginX * 2 ;
    ...
    // 只有 Menu 是固定长度时才进行判断
    if (_menu.isMenuWidthFixed) {
        if (maxItemWidth > _menu.menuFixedWidth) {
            maxItemWidth = _menu.menuFixedWidth;
        }
    }
    
//    const CGFloat titleX = kMarginX * 2 + (maxImageWidth > 0 ? maxImageWidth + kMarginX : 0);
    const CGFloat titleX = kMarginX * 2 + (maxImageWidth > 0 ? maxImageWidth : 0);
    ...
    for (LDPMPopoverMenuItem *menuItem in _menuItems) {
        ...
        if (menuItem.attachmentView) {
            const CGRect attachmentViewFrame = CGRectMake(maxItemWidth - kMarginX + kMarginTextToAttachView,
                                                          (maxItemHeight - menuItem.attachmentView.size.height) / 2.0,
                                                          menuItem.attachmentView.size.width,
                                                          menuItem.attachmentView.size.height);
            menuItem.attachmentView.frame = attachmentViewFrame;
            menuItem.attachmentView.autoresizingMask = UIViewAutoresizingNone;
            [itemView addSubview:menuItem.attachmentView];
        }
